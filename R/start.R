...
get_within_parent_gate <- function(parent_gate = 'Time/S/Lv/K1/K2/K3/K4/K5/K6/K7/K8/14-/S/L/19-/3+/3+excl 16br/56-16-/4+'){
  parts = stringr::str_split(parent_gate , pattern = fixed("/"))
  return(parts[[1]])
}
...
get_child_paths <- function(fcm08_filepath        = params$fcm08_path,
  ...
  child_paths = all_paths[all_paths %>% stringr::str_detect(., pattern = fixed(parent_gate))]
  return(child_paths)
  
}

extract_last_element <- function(x){
  split_string <- stringr::str_split(string = x, pattern = fixed("/"))[[1]]
